...
- The next critical Rig intelligence milestone is to finish **Phase 4 / `RIG-401` orchestration** by tightening the remaining failure and verification edges around action execution.
- The next `RIG-401` sub-slice after this should focus on stage-aware failure provenance for action verify/persist paths before starting `RIG-402`.
