 ...
 */
const isSelfConnection = (source: string, target: string): boolean => source === target;
...
const createEdgeId = (existingEdges: Edge[]): string => {
...
export const createValidatedEdge = (
  ...
  pipe(
    ...
    Effect.map(() => createEdge(source, target, label, undefined, edges)),
  );
...
export const updateEdgeLabel = (
  ...
  pipe(
    ...
    Effect.map((validatedLabel) => edges.map((e) => (e.id === edgeId ? { ...e, label: validatedLabel } : e))),
  );
