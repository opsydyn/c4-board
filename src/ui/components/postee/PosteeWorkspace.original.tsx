import type { PosteeEnvironmentVariable, PosteeHistoryEntry } from "@/core/effects/database.postee";
import {
  CaretDownIcon,
  CaretLeftIcon,
  CaretRightIcon,
...
export function PosteeWorkspace() {
  ...
  const requestStatusMap = useMemo(() => {
    ...
    for (const entry of history) {
      ...
      const status: RequestStatus = entry.error_message
  ...
  const selectedTreeKey = activeRequestId !== null && activeRequestId !== undefined
  ...
  const rightTrack = !isCompactLayout && isResponseOpen ? rightTrackBase : "0px";
              ...
              : (
                ...
                >
                  {collections.map((collection: { id: string; name: string }) => {
                    ...
                    const folderStatus = collectionStatusMap.get(collection.id) ?? "unknown";
                            ...
                            >
                              {collectionRequests.length > 0
                                ? (
                                  <Button
                                    slot="chevron"
                                    aria-label={itemProps.isExpanded
                              ...
                              <div className={styles.treeItemLabel}>
                                ...
                                <div className={styles.treeLabelContent}>
                                  {isRenamingCollectionId === collection.id
                                    ? (
                                      <InlineEditor
                                        ...
                                        maxLength={120}
                                        onSave={(value) => handleRenameSave(collection.id, value)}
                                  ...
                                  <span className={styles.treeCountBadge}>
                                    {collectionRequests.length} {collectionRequests.length === 1
                                      ? "request"
                                      : "requests"}
                                  </span>
                                </div>
        ...
        <header className={styles.mainHeader}>
          ...
          <div className={styles.mainHeaderActions}>
            <ToggleButton
              isSelected={isResponseOpen}
              onChange={setResponseOpen}
              aria-label={isResponseOpen ? "Hide response panel" : "Show response panel"}
            >
              <CaretRightIcon size={16} weight="bold" />
              Response
            </ToggleButton>
