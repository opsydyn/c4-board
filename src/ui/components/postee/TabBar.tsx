...
import { tab, tabActive, tabBar } from "./TabBar.css";
...
export function TabBar({ tabs, activeTab, onTabChange, children }: TabBarProps) {
  const tabItems = tabs.map((item) => typeof item === "string" ? { id: item, label: item } : item);
      ...
      <TabList className={tabBar}>
        {tabItems.map((tabItem) => (
          <Tab
            key={tabItem.id}
            id={tabItem.id}
            className={({ isSelected }) => isSelected ? `${tab} ${tabActive}` : tab}
          >
            {tabItem.label}
          </Tab>
        ))}
