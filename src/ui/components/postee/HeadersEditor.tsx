...
import { Button, TextField, Checkbox, Input } from "react-aria-components";
import {
	headersContainer,
	headerRow,
	headerCheckbox,
			...
			{headers.map((header) => (
				<div key={header.id} className={headerRow}>
					<Checkbox
						...
						onChange={(isSelected) =>
							handleUpdateHeader(header.id, { enabled: isSelected })
						}
					...
					>
						<Input
							className={`${headerInput} ${headerKeyInput}`}
					...
					>
						<Input
							className={`${headerInput} ${headerValueInput}`}
							placeholder="Value"
						/>
					</TextField>
