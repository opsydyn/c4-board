...
import type { C4Type, DDDType, NodeData, NodeType } from "../../core/effects/node-operations";
...
const useEvolutionModel = (nodes: Node[], edges: Edge[], domain: DiagramDomain): EvolutionModel =>
  useMemo(() => {
    ...
    const edgeEvents: EvolutionEvent[] = edges.map((edge, index) => {
      ...
      const sourceLabel = typeof nodeMap.get(edge.source)?.data?.label === "string"
        ? (nodeMap.get(edge.source)?.data?.label as string)
        : edge.source;
      const targetLabel = typeof nodeMap.get(edge.target)?.data?.label === "string"
    ...
    const latestEvent: EvolutionEvent | null = combined.length > 0 ? combined[combined.length - 1] ?? null : null;
    ...
    <div className={evolutionCard}>
      ...
      <div className={evolutionChart}>
        <ParentSize debounceTime={80}>
          {({ width, height }) => <EvolutionViz points={model.points} width={width} height={height} domain={domain} />}
        </ParentSize>
      </div>
